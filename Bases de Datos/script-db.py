...
try:
    ...
    print(f"ERROR: No se pudo cargar el archivo config.json: {e}")
    print("Asegúrate de crear un archivo 'config.json' con el formato correcto.")
    exit(1)
...
def generar_informe_diario():
    ...
    try:
        ...
        datos_hoy = pd.read_sql_query(consulta, conn) #Leer query
        conn.close()
...
if __name__ == "__main__":
    ...
    if not (broker and dni and usuario and contrasena):
        print("ERROR: Faltan credenciales en el archivo config.json")
        exit(1)
